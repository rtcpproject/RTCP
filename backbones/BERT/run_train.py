...
from transformers import BertForSequenceClassification, BertTokenizer, AdamW, WEIGHTS_NAME, CONFIG_NAME, GPT2Tokenizer
from torch.utils.data import DataLoader
from data_utils import load_data, NEW_ADD_TOKENS
# from data_utils_full_info import load_data, NEW_ADD_TOKENS
from dataset_bert import BERTDataset, IGNORE_INDEX

from urllib3 import ProxyManager, make_headers
default_headers = make_headers(proxy_basic_auth='huydq44:Tsuchimikadoharu140598')
http = ProxyManager("http://10.16.29.21:8080/", headers=default_headers)

import wandb
wandb.login(key="5b372cb7e06d74f891cdbc6271dd7fee81d984b3")
config = wandb.config
...
def train():
    ...
    # wandb.init(project='English_Conversational_Planning', name = f"Reward_Training_{args.random_seed}")
    # wandb.watch(model)

    # Prepare datasets
    logger.info("Prepare datasets")
    train_data = load_data(tokenizer, logger, args.train_path, args.cache_dir, 
        data_partition="train", use_tcp=False)
    valid_data = load_data(tokenizer, logger, args.valid_path, args.cache_dir, 
        data_partition="valid", use_tcp=False)
    
    train_dataset = BERTDataset(train_data, tokenizer, max_seq_len=args.max_seq_len)
    valid_dataset = BERTDataset(valid_data, tokenizer, max_seq_len=args.max_seq_len)
    ...
    def update(engine, batch):
        ...
        ##wandb logging
        # wandb.log({'train/Gen_loss': loss.item()})
