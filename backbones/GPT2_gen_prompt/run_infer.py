...
from transformers import BertTokenizer, GPT2LMHeadModel, GPT2Tokenizer, GPT2Config
...
def main():
    ...
    parser.add_argument("--checkpoint_name", type=str, default="", help="Path, url or short name of the model")
    ...
    # ### read the action/topic vocabulary
    # all_topics_path = 'data/all_topics.pkl'
    # all_actions_path = 'data/all_goals.pkl'
    # all_actions = read_binary_file(all_actions_path)
    # all_topics = read_binary_file(all_topics_path)

    model_dir = args.model_dir
    logger.info("Loading tokenizer from [{}]".format(model_dir))
    # tokenizer = GPT2Tokenizer.from_pretrained(model_dir)
    # plm = GPT2LMHeadModel.from_pretrained(model_dir)
    ...
    model.load_state_dict(torch.load(os.path.join(args.model_dir, args.checkpoint_name)))
    ...
    ppl_path = os.path.join(output_dir, "ppl.txt")

    with open(ppl_path, 'w') as f:
        f.writelines(str(np.math.exp(avg_loss)))
