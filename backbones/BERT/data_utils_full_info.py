...
def convert_data(fp, extract_kg=False, tcp_path=None):
    ...
    with open(fp, 'rb') as f:
        ...
        for idx, sample in enumerate(fr):
            ...
            ### get the current action and topic
            prev_action = sample['goals'][-1]
            prev_topic = sample['topics'][-1]

            ### target action and target topic
            ...
            ### attend the current action and topic to the query
            input_str += ACTION + prev_action + TOPIC + prev_topic + SEP

            ### append the target action and target topic to the query
